...
namespace OsuRTDataProvider
{
    ...
    public class OsuRTDataProviderPlugin : Plugin
    {
        ...
        private Logger<OsuRTDataProviderPlugin> logger = new Logger<OsuRTDataProviderPlugin>();

        public OsuRTDataProviderPlugin() : base(PLUGIN_NAME, PLUGIN_AUTHOR)
        {
            // Init Logger
            OsuRTDataProvider.Logger.OnInfo = logger.LogInfomation;
            OsuRTDataProvider.Logger.OnDebug = logger.LogInfomation;
            OsuRTDataProvider.Logger.OnError = logger.LogError;
            OsuRTDataProvider.Logger.OnWarn = logger.LogWarning;

            // Init Language
            var lang = new PluginLanguage();
            ...
            base.EventBus.BindEvent<PluginEvents.ProgramReadyEvent>((e) => {
                logger.LogInfomation(string.Format(PluginLanguage.LANG_TOURNEY_HINT, Setting.EnableTourneyMode));
            ...
            @e.Commands.Dispatch.bind("ortdp", (args) =>
             {
                 ...
                 if (args.Count >= 2)
                 {
                     if (args[0] == "debug")
                     {
                         ...
                         {
                             DebugOutput(f);
                             logger.LogInfomation($"Debug mode = {Setting.DebugMode}");
                         }
                     }
                     return true;
                 }
        ...
        private void DebugOutput(bool enable, bool first = false)
        {
            ...
            if (Setting.EnableTourneyMode)
            {
                ...
                {
                    int id = i;
                    void OnTourneyStatusChanged(OsuListenerManager.OsuStatus l, OsuListenerManager.OsuStatus c)=>
                        logger.LogInfomation($"[{id}]Current Game Status:{c}");
                    void OnTourneyModsChanged(ModsInfo m)=>
                        logger.LogInfomation($"[{id}]Mods:{m}(0x{(uint)m.Mod:X8})");
                    void OnTourneyModeChanged(OsuPlayMode last, OsuPlayMode mode)=>
                        logger.LogInfomation($"[{id}]Mode:{mode}");
                    void OnTourneyBeatmapChanged(Beatmap map) =>
                        logger.LogInfomation($"[{id}]Beatmap: {map.Artist} - {map.Title}[{map.Difficulty}]({map.BeatmapSetID},{map.BeatmapID},{map.FilenameFull})");
                    void OnTourneyPlayerChanged(string playername) =>
                        logger.LogInfomation($"[{id}]Current Player: {playername}");
                    /*
            ...
            else
            {
                void OnStatusChanged(OsuListenerManager.OsuStatus l, OsuListenerManager.OsuStatus c) =>
                    logger.LogInfomation($"Current Game Status:{c}");
                void OnModsChanged(ModsInfo m) =>
                    logger.LogInfomation($"Mods:{m}(0x{(uint)m.Mod:X8})");
                void OnModeChanged(OsuPlayMode last, OsuPlayMode mode) =>
                    logger.LogInfomation($"Mode:{mode}");
                void OnBeatmapChanged(Beatmap map) =>
                    logger.LogInfomation($"Beatmap: {map.Artist} - {map.Title}[{map.Difficulty}]({map.BeatmapSetID},{map.BeatmapID},{map.FilenameFull})");
                void OnPlayerChanged(string playername) =>
                    logger.LogInfomation($"Current Player: {playername}");
                /*
